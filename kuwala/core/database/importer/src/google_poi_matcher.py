...
def import_matches(
    spark, database_url, database_properties, search_results_dir, poi_data_dir, basis
):
    search_results_files = sorted(
        filter(lambda f: basis in f and "matched" in f, os.listdir(search_results_dir)),
        reverse=True,
    )
    poi_data_files = sorted(
        filter(lambda f: basis in f and "matched" in f, os.listdir(poi_data_dir)),
        reverse=True,
    )
    ...
    df_poi_data = (
        spark.read.parquet(file_path_poi_data)
        .dropDuplicates(["internal_id"])
        .select("internal_id")
    )
    df_search_results = df_search_results.join(
        df_poi_data, on="internal_id", how="inner"
    )

    if basis == "custom":
        df_search_results = df_search_results.withColumnRenamed(
            "id", "custom_id"
        ).dropDuplicates(["custom_id"])

    df_search_results.write.option("truncate", True).jdbc(
        url=database_url,
        table=f"google_{basis}_poi_matching",
        mode="overwrite",
        properties=database_properties,
    )


def import_google_poi_matching_data(
    spark, database_url, database_properties, continent, country, country_region
):
    start_time = time.time()

    logging.info(
        f"Starting import of Google-POI matching data for "
    ...
    if not (os.path.exists(search_results_dir) and os.path.exists(poi_data_dir)):
        logging.warning(
            "Search results and poi data are not both available. Skipping import."
        )
        return
    ...
    logging.info(
        f"Successfully imported Google-POI matching data for "
