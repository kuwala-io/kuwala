...
def import_google_pois(spark, database_url, database_properties, continent, country, country_region):
    ...
    opening_hours_data = data.select('internal_id', 'opening_hours') \
        ...
        .withColumn('opening_time', regexp_replace('opening_time', '\\.', ':').cast(TimestampType())) \
        .withColumn('closing_time', regexp_replace('closing_time', '\\.', ':').cast(TimestampType()))
    ...
    popularity_data.write.option('truncate', True) \
    ...
    opening_hours_data.write.option('truncate', True) \
    ...
    waiting_time_data.write.option('truncate', True) \
