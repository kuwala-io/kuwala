...
def import_osm_pois(
    ...
    logging.info(
        f'Starting import of OSM POIs for {f"{country_region}, " if country_region else ""}'
        f"{country}, {continent}"
    )
    ...
    data = (
        ...
        .withColumn("tags", concat_tags(col("tags")))
        .withColumn("kuwala_import_country", lit(country))
    )

    data.write.option("truncate", True).option("cascadeTruncate", True).jdbc(
        url=database_url,
        table="osm_poi",
        mode="overwrite",
    ...
    logging.info(
        f'Successfully imported OSM POIs for {f"{country_region}, " if country_region else ""}{country}, '
