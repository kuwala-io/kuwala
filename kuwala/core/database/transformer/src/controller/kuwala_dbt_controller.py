...
class KuwalaDbtController:
    def __init__(self, dbt_path: str, dbt_host: str, result_path: str):
        ...
        if ssh_host and ssh_user and ssh_pkey:
            self.ssh_tunnel = SSHTunnelForwarder(ssh_host, ssh_username=ssh_user, ssh_pkey=ssh_pkey,
                                                 remote_bind_address=('127.0.0.1', 5432))

            self.ssh_tunnel.start()
            self.shell_environment = dict(os.environ, DBT_PORT=str(self.ssh_tunnel.local_bind_port), DBT_HOST=dbt_host)
        else:
    ...
    def run_all_models(self):
        ...
        subprocess.call('dbt run --profiles-dir .', shell=True, env=self.shell_environment)
