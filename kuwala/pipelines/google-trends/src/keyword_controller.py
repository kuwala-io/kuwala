...
def get_keyword_by_region(sp, continent, country, country_region, keyword):
    ...
    if not os.path.exists(file_path):
        logging.error(
            "No admin boundaries have been processed. Please run the admin-boundaries pipeline first."
        )
    ...
    keywords = admin_boundaries.withColumn(
        "keyword", concat_ws(" ", lit(keyword), col("name"))
    )
    ...
    if country_region:
        geo = list(
            map(
                lambda s: dict(
                    confidence=fuzz.token_set_ratio(country_region, s.name), id=s.code
                ),
                subdivisions,
            )
        )
